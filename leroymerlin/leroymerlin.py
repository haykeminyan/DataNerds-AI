...
def switcher(href_addr):
    while True:
        try:
            ...
            continue
    return ip
logging.basicConfig(filename="leroymerlin.log",format='%(asctime)s - %(message)s',datefmt='%d-%b-%y %H:%M:%S',filemode='w', level=logging.INFO)
...
def parse_single_html(href_addr):
    ...
    leroy_main_count_pages_1=leroy_main_soup_1.find_all(class_='paginator-item')
    arr_count_pages_bad=[]
    arr_count_pages=[]
...
for i,j in arr_names_href_0_level.items():
    ...
    html_heading=html_soup.find_all(class_='items-border section-card__items')
