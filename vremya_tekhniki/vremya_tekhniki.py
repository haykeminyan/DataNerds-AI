...
def switcher(href_addr):
    while True:
        try:
            ...
            continue
    return ip
logging.basicConfig(filename="tekh.log",format='%(asctime)s - %(message)s',datefmt='%d-%b-%y %H:%M:%S',filemode='w', level=logging.INFO)
tekh_main_page=switcher(href_addr)
tekh_main_soup=BeautifulSoup(tekh_main_page,'html.parser')
tekh_main_heading=tekh_main_soup.find_all(class_='cs-product-groups-gallery__image-link')
arr_names=[]
arr_href=[]
...
logging.info('The length of names 1 level Category is: {}'.format(len(arr_names)))
...
def parse_single_html(href_addr):
    ...
    for i in range(1,page+1):
        ...
        tekh_price=tekh_soup.find_all(class_='cs-goods-price__value cs-goods-price__value_type_current')
        arr_price_bad=[]
        for c in tekh_price:
            d=c.text.strip()
            j=d.split('\xa0')
        ...
        counter+=1
    logging.info('--------------------------------------------------------------------------------------------------------------------------------------------------')
    logging.info('{:<11} #Pages: {:<11}  #Goods:{:<11}'.format(str(href_addr),str(page),str(len(res))))
    return res
...
for i,j in arr_names_href_1_level.items():
    ...
    html_heading=html_soup.find_all(class_='cs-product-groups-gallery__image-link')
