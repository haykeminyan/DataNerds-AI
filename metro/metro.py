...
logging.basicConfig(filename="metro.log",format='%(asctime)s - %(message)s',datefmt='%d-%b-%y %H:%M:%S',filemode='w', level=logging.INFO)
...
for c in range(len(category_name)):
    ...
    if k not in b:
        ...
        b[k]+=1
        logging.info('--------------------------------------------------------------------------------------------------------------------------------------------------')
        logging.error('We have similar {} <<category_1_level={}>> !!!!!'.format(b[k],k))
dict_main_1_level={}
...
for k,m in dict_main_1_level.items():
    ...
    href_2_level=[]
    main_heading_2_level=list(main_soup_two.find_all(class_='subcatalog cat1'))
    ...
    for c in range(len(category_name_2_level)):
        ...
        if k not in a:
            ...
            a[k]+=1
            logging.info('--------------------------------------------------------------------------------------------------------------------------------------------------')
            logging.error('We have similar {} <<category_2_level={}>> !!!!!!!!!!!!!'.format(a[k],k))
...
def parse_single_html(href_addr):
    ...
    for page in range(1,pages+1):
        html_page=href_addr+'?price_range=11%3B601&brands=&attrs=&attr[5163][from]=0&attr[5163][to]=0&sorting=0&limit=12&in_stock=0&virtual_stock=0&page='+str(page)
        print(html_page)
        main_page_goods=switcher(html_page)
        main_soup_goods=BeautifulSoup(main_page_goods,'html.parser')
        goods_names_soup=list(main_soup_goods.find_all(class_='catalog-i_image'))
        goods_names=[]
        goods_cost=[]
        ...
        for c in range(len(goods_names)):
            ...
            if k not in a:
                ...
                a[k]+=1
                logging.info('--------------------------------------------------------------------------------------------------------------------------------------------------')
                logging.error('We have similar {} <<name={}>> in {}  page!!!!!!!!!!!!!'.format(a[k],k,page))
    ...
    logging.info('{:<11} #Pages: {:<11}  #Goods:{:<11}'.format(str(href_addr),str(pages),str(len(result))))
    print(result)
    return result
...
for i,j in dict_main_2_level.items():
    ...
    for k,m in j.items():
        ...
        logging.info('--------------------------------------------------------------------------------------------------------------------------------------------------')
        counter_2+=1
        db.final_metro.update_one({'category_name':k, 'link':m},{'$set':{date:final_m[date]}},upsert=True)
