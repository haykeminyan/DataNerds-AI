...
logging.basicConfig(filename="/Users/datanerds/workspace/perekrestok/perekrestok_2.log",format='%(asctime)s - %(message)s',datefmt='%d-%b-%y %H:%M:%S',filemode='w', level=logging.INFO)
...
perekrestok_heading_1_catogories=perekrestok_main_soup.find_all(class_='xf-catalog-categories__list _list')
arr=[]
...
arr_name_href_1_level=dict(zip(arr_name_category_1_level,arr_href_category_1_level))
print(arr_name_href_1_level)

logging.info('The length of array with categories_names 1 level is: '+str(len(arr_name_category_1_level)))
logging.info('The length of array with categories_href 1 level is: '+str(len(arr_href_category_1_level)))
...
def parse_single_html(href_addr):
    ...
    html_goods_2_heading=html_goods_2_soup.find_all(class_='js-list-total__total-count')
    ...
    for number in range(1,page+1):
        ...
        id_items=soup_one.find_all(class_='catalog__items-wrap js-catalog-wrap')
        for a in id_items:
            for f in a.find_all(class_='xf-product js-product'):
                arr_items.append(f.get('data-gtm-product-name').replace('.',','))
        a={}        
        for c in range(len(arr_items)):
            ...
            if k not in a:
                ...
                a[k]+=1
                logging.info('--------------------------------------------------------------------------------------------------------------------------------------------------')
                logging.error('We have similar {} <<name={}>> in {}  page!!!!!!!!!!!!!'.format(a[k],k,number))
        arr_rouble=[]
        arr_penny=[]
        arr_cost_bad=[]
        for b in id_items:
            for q in b.find_all(class_='xf-product__cost xf-product-cost'):
                k=q.find(class_='xf-price__rouble js-price-rouble').text.strip()
                arr_rouble.append(k)
                m=q.find(class_='xf-price__penny js-price-penny').text.strip()
        ...
        counter+=1
        # logging.info('--------------------------------------------------------------------------------------------------------------------------------------------------')
    logging.info('#Pages:{:<2} #Goods:{:<11}'.format(str(number),str(len(res_1))))
    return res_1
...
for i,j in arr_name_href_1_level.items():
    ...
    perekrestok_main_soup_2_=BeautifulSoup(perekrestok_main_page_2_,'html.parser')
    perekrestok_heading_2_catogories=perekrestok_main_soup_2_.find_all(class_='xf-filter__item-label xf-ripple js-xf-ripple xf-ripple_gray')
...
for i,j in dict_1.items():
    ...
    for k,m in j.items():
        ...
        logging.info('#Category_2_name:  {:<11} Counter_2_level: {:<11} #href_2_level:{:<11}'.format(str(k),str(counter_2),str(m)))
        logging.info('--------------------------------------------------------------------------------------------------------------------------------------------------')
        counter_2+=1
        print(final_p)
        db.final_perekrestok.update_one({'category_name':k, 'link':m},{'$set':{date:final_p[date]}},upsert=True)
