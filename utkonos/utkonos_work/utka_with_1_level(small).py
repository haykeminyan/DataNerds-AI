...
logging.info('The length of array with categories_names is: '+str(len(arr_name_submenu_final)))
logging.info('The length of array with categories_href is: '+str(len(arr_href_submenu_final)))
    ...
    for k in range(1,page+1):
        ...
        for i in base_0:
            ...
            for j in i.find_all(class_='goods_price'):
                arr_cost_1.append(j.get("data-static-now-price").replace(',','.'))
        ...
        for c in range(len(arr_names)):
            ...
            if z not in a:
                ...
                a[z]+=1
                logging.info('--------------------------------------------------------------------------------------------------------------------------------------------------')
                logging.error('We have similar {} <<name={}>> in {}   page!!!!!!!!!!!!!'.format(a[z],z,k))
        for w in arr_cost_1:
...
for i,j in arr_name_href_submenu.items():
    ...
    counter+=1
    f.write(str(final))
    db.final_utkonos.update_one({'category_name':i, 'link':j},{'$set':{date:final[date]}},upsert=True)
