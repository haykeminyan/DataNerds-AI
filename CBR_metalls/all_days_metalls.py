...
import schedule
import time
logging.basicConfig(filename="all_days.log", format='%(asctime)s - %(message)s',
                    datefmt='%d-%b-%y %H:%M:%S', filemode='w', level=logging.INFO)
logging.getLogger('requests').setLevel(logging.ERROR)
current_date = datetime.today().strftime("%d.%m.%Y")
href_addr = 'http://www.cbr.ru/hd_base/metall/metall_base_new/?UniDbQuery.Posted=True&UniDbQuery.Gold=true&UniDbQuery.Silver=true&UniDbQuery.Platinum=true&UniDbQuery.Palladium=true&UniDbQuery.FromDate=01.07.2008&UniDbQuery.ToDate=' + \
    str(current_date)
print(href_addr)
