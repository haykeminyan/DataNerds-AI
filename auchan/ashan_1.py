...
logging.basicConfig(filename="/Users/datanerds/workspace/auchan/ashan_for_soft.log",format='%(asctime)s - %(message)s',datefmt='%d-%b-%y %H:%M:%S',filemode='w', level=logging.INFO)
...
ashan_heading_0_level_href=ashan_main_soup_0_level.find_all(class_='m-menu__title')
...
arr_name_href_submenu_0_level=dict(zip(arr_name_submenu_0_level,arr_href_submenu_0_level))

logging.info('#Length 1 level categories {:<11}'.format(str(len(arr_name_submenu_0_level))))
logging.info('#Length 1 level href''s {:<11}'.format(str(len(arr_href_submenu_0_level))))
...
def parse_single_html(href_addr):
    ...
    html_goods_2_main_heading=html_goods_2_main_soup.find_all(class_='pagination__item')
    ...
    for i in range(1,page+1):
        ...
        ashan_heading=ashan_soup.find_all(class_='products__list products__list--grid')
        arr_names=[]
        ...
        for c in range(len(arr_names_final)):
            ...
            if k not in a:
                ...
                a[k]+=1
                logging.info('--------------------------------------------------------------------------------------------------------------------------------------------------')
                logging.error('We have similar {} <<name={}>> in {}  page!!!!!!!!!!!!!'.format(a[k],k,i))
        arr_cost=[]
        arr_cost_final=[]
        arr_cost_final_int=[]
        ...
        counter+=1
    logging.info('{:<11} #Pages: {:<11}  #Goods:{:<11}'.format(str(href_addr),str(page),str(len(res_1))))
    return res_1
...
for x,y in arr_name_href_submenu_0_level.items():
    ...
    ashan_heading_1_level=ashan_main_soup.find_all(class_='category__item-title')
...
for i,j in dict_1.items():
    ...
    for k,m in j.items():
        ...
        logging.info('--------------------------------------------------------------------------------------------------------------------------------------------------')
        counter_2+=1
        db.final_auchan.update_one({'category_name':k, 'link':m},{'$set':{date:final_a[date]}},upsert=True)
