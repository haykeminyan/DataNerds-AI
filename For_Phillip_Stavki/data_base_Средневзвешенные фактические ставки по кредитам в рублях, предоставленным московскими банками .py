...
logging.basicConfig(filename="data_base_srednevz.log",format='%(asctime)s - %(message)s',datefmt='%d-%b-%y %H:%M:%S',filemode='w', level=logging.INFO)
logging.getLogger('requests').setLevel(logging.ERROR)

current_date = datetime.today().strftime("%d.%m.%Y")
yesterday_fulltime=datetime.today()-timedelta(1)
yesterday=yesterday_fulltime.strftime("%d.%m.%Y")
href_addr='http://www.cbr.ru/hd_base/mkr/mkr_base/?UniDbQuery.Posted=True&UniDbQuery.st=SF&UniDbQuery.ob=OB_MIACR_0&UniDbQuery.Currency=-1&UniDbQuery.sk=Dd1_&UniDbQuery.FromDate=31.10.1994&UniDbQuery.ToDate='+str(current_date)
