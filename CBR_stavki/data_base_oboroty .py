...
logging.basicConfig(filename="database_oboroty.log", format='%(asctime)s - %(message)s',
                    datefmt='%d-%b-%y %H:%M:%S', filemode='w', level=logging.INFO)
logging.getLogger('requests').setLevel(logging.ERROR)

current_date = datetime.today().strftime("%d.%m.%Y")
href_addr = 'http://www.cbr.ru/hd_base/mkr/mkr_base/?UniDbQuery.Posted=True&UniDbQuery.st=SF&UniDbQuery.ob=OB_MIACR_0&UniDbQuery.Currency=-1&UniDbQuery.sk=Dd1_&UniDbQuery.FromDate=31.10.1994&UniDbQuery.ToDate='

directory = os.path.abspath(os.getcwd())
folder_name = 'current_day_oboroty'
...
print(current_date)
href_addr_current_date = 'http://www.cbr.ru/hd_base/mkr/mkr_base/?UniDbQuery.Posted=True&UniDbQuery.st=SF&UniDbQuery.ob=OB_MIACR_0&UniDbQuery.Currency=-1&UniDbQuery.sk=Dd1_&UniDbQuery.FromDate=31.10.1994&UniDbQuery.ToDate='+current_date
...
columns_1 = 2
a_current = list(main_soup_current.select(
    'div.table-wrapper:nth-child(6) > div:nth-child(2)'))
for i in a_current:
    j = i.text.strip()
    k_current = j.split('\n')
