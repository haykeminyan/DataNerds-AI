...
logging.basicConfig(filename="data_base_srednevz.log", format='%(asctime)s - %(message)s',
...
folder_name = 'data_base_Средневзвешенные фактические ставки по кредитам в рублях, предоставленным московскими банками '
...
columns = 2
a = list(main_soup.select(
    'div.table-wrapper:nth-child(2) > div:nth-child(2) > table:nth-child(1)'))
for i in a:
    j = i.text.strip()
    k = j.split('\n')
k = [i for i in k if i]
