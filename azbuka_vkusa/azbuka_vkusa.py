...
logging.basicConfig(filename="azbuka_vkusa.log",format='%(asctime)s - %(message)s',datefmt='%d-%b-%y %H:%M:%S',filemode='w', level=logging.INFO)
...
heading_1=list(main_soup.find_all(class_='b-header-menu__level-item js-show-menu b-header-menu__level-item_empty'))
heading_2=list(main_soup.find_all(class_='b-header-menu__level-item js-show-menu'))
arr_1_names=[]
arr_1_href=[]
arr_2_names=[]
arr_2_href=[]
...
def parsing_single_html(href_addr):
    ...
    for i in range(pages):
        html_addr=href_addr+'?q=%3Aacbestseller%3AassortmentTypes%3AINTERNET&page='+str(i)
        html_page=switcher(html_addr)
        azbuka_soup_1=BeautifulSoup(html_page,'html.parser')
        arr_names=[]
        heading_names=list(azbuka_soup_1.find_all(class_='b-product__title js-list-prod-open'))
        ...
        for b in range(len(arr_names)):
            ...
            if k not in a:
                ...
                a[k]+=1
                logging.info('--------------------------------------------------------------------------------------------------')
                logging.error('We have similar {:<3} name_goods={} in {} page!'.format(a[k],k,i+1))
                logging.info('--------------------------------------------------------------------------------------------------')
        heading_price=list(azbuka_soup_1.find_all(class_='b-product__price-block'))
        ...
        for c in heading_price:
            for d in c.find_all(True, {'class':['b-product__price b-price b-price_product', 'b-product__price b-price b-price_product b-price_product-red']}):
                arr_price_with_and_without_discount.append(float(d.get('content').replace('\xa0','')))
        arr_items=dict(zip(arr_names,arr_price_with_and_without_discount))
        res={**res,**arr_items}
    logging.info('--------------------------------------------------------------------------------------------------')
    logging.info('#Pages: {:<11} href_addr: {:<11} counter_goods: {:<11}'.format(pages,href_addr,len(res)))
    return res
